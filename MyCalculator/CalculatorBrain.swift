...
//  MyCalculator
//
//  Created by Sergey Mikhailov on 23/01/2019.
...
struct CalculatorBrain {
    ...
    private let errorOperations: Dictionary<String,ErrorOperation> = [
        "√": ErrorOperation.unaryOperation({ 0.0 > $0 ? "Корень отрицательного числа" : nil }),
        "÷": ErrorOperation.binaryOperation({ 1e-8 >= fabs($1) ? "Деление на 0" : nil }),
        "x⁻¹" : ErrorOperation.unaryOperation({ 1e-8 > fabs($0) ? "Деление на 0" : nil }),
        "ln" : ErrorOperation.unaryOperation({ 0 > $0 ? "ln Отрицательного числа" : nil }),
        "log" : ErrorOperation.unaryOperation({ 0 > $0 ? "log Отрицательного числа" : nil }),
        "x!" : ErrorOperation.unaryOperation({ 0 > $0 ? "Factorial Отрицательного числа" : nil })
    ]
